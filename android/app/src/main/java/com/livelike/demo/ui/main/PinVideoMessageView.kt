...
import com.livelike.demo.databinding.PinVideoMessageBinding
...
class PinVideoMessageView @JvmOverloads constructor(
    ...
    var _binding: PinVideoMessageBinding? = null
    ...
    private fun inflate(context: Context) {
        if (!inflated) {
            inflated = true
            _binding = PinVideoMessageBinding.bind(
                inflate(
                    context,
                    R.layout.pin_video_message,
                    this@PinVideoMessageView
                )
            )
        }
            ...
            try {
                ...
                try {
                    ...
                    it.playerView.setOnCompletionListener { mediaPlayer ->
                        ...
                        setFrameThumbnail(videoUrl)
                        it.soundView.visibility = LinearLayout.GONE
                    }

                    it.playerView.setOnErrorListener { _, what, extra ->
                        hideVideoControls()
                        it.progressBar.visibility = LinearLayout.GONE
                        it.playerView.visibility = LinearLayout.INVISIBLE
                        it.playbackErrorView.visibility = LinearLayout.VISIBLE
                        true
                    }
                } catch (e: Exception) {
                    it.progressBar.visibility = LinearLayout.GONE
    ...
    private fun showVideoControls(){
        _binding?.let {
            ...
            it.icPlay.visibility = LinearLayout.VISIBLE
            it.playbackErrorView.visibility = LinearLayout.GONE
            it.soundView.visibility = LinearLayout.VISIBLE
        }
    }
    ...
    private fun hideVideoControls() {
        _binding?.let {
            it.icPlay.setImageResource(R.drawable.ic_play_button)
            it.icPlay.visibility = LinearLayout.GONE
            it.playbackErrorView.visibility = LinearLayout.GONE
            it.soundView.visibility = LinearLayout.GONE
        }
    }
    ...
    private fun setFrameThumbnail(videoUrl: String) {
        _binding?.let {
            it.thumbnailView.visibility = LinearLayout.VISIBLE
            showVideoControls()
            it.progressBar.visibility = LinearLayout.GONE
            it.playbackErrorView.visibility = LinearLayout.GONE
            it.playerView.visibility = LinearLayout.INVISIBLE
